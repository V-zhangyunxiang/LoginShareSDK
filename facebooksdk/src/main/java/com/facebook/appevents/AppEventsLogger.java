...
public class AppEventsLogger {
    ...
    private static final String APP_EVENT_NAME_PUSH_ARRIVAL = "fb_mobile_push_arrival";
    private static final String APP_EVENT_NAME_PUSH_ARRIVAL_REPEAT = "fb_mobile_push_arrival_repeat";   //重复推送的事件
    ...
    /**
     * Logs an app event that tracks that the Push Notification was arrival to user.
     * @param payload Notification payload received.
     */
    public void logPushNotificationArrival(Bundle payload) {
        String campaignId = null;
        try {
            String payloadString = payload.getString(PUSH_PAYLOAD_KEY);
            if (Utility.isNullOrEmpty(payloadString)) {
        ...
        if (campaignId == null) {
            Logger.log(LoggingBehavior.DEVELOPER_ERRORS, TAG,
                    "Malformed payload specified for logging a push notification arrival.");
            return;
        }

        Bundle parameters = new Bundle();
        parameters.putString(APP_EVENT_PUSH_PARAMETER_CAMPAIGN, campaignId);
        logEvent(APP_EVENT_NAME_PUSH_ARRIVAL, parameters);
    }

    /**
     * Logs an app event that tracks that the Push Notification was !!!***repeat***!!! arrival to user.
     * @param payload Notification payload received.
     */
    public void logPushNotificationArrivalRepeat(Bundle payload) {
        String campaignId = null;
        try {
            String payloadString = payload.getString(PUSH_PAYLOAD_KEY);
            if (Utility.isNullOrEmpty(payloadString)) {
        ...
        Bundle parameters = new Bundle();
        parameters.putString(APP_EVENT_PUSH_PARAMETER_CAMPAIGN, campaignId);
        logEvent(APP_EVENT_NAME_PUSH_ARRIVAL_REPEAT, parameters);
    }
